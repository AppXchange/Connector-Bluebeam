...
public class SubscribeStudioProjectSubscriptionHandler : IActionHandler<SubscribeStudioProjectSubscriptionAction>
{
    private readonly ILogger<SubscribeStudioProjectSubscriptionHandler> _logger;
    private readonly ApiClient _apiClient;

    public SubscribeStudioProjectSubscriptionHandler(
        ILogger<SubscribeStudioProjectSubscriptionHandler> logger,
        ApiClient apiClient)
    {
        _logger = logger;
    ...
    {
        var input = JsonSerializer.Deserialize<SubscribeStudioProjectSubscriptionActionInput>(actionInstance.InputJson);
        if (input == null)
        {
            return ActionHandlerOutcome.Failed(new StandardActionFailure
            {
                ...
                {
                    Source = new[] { "SubscribeStudioProjectSubscriptionHandler" },
        ...
        try
        {
            var response = await _apiClient.SubscribeToStudioProject(input, cancellationToken);
            if (!response.IsSuccessful)
            {
                return ActionHandlerOutcome.Failed(new StandardActionFailure
                {
                    ...
                    {
                        Source = new[] { "SubscribeStudioProjectSubscriptionHandler" },
                        Text = "Failed to subscribe to Studio Project"
                    }}
                });
            }
        ...
        {
            _logger.LogError(exception, "Failed to subscribe to Studio Project");
